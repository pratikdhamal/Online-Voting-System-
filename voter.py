...
def failed_return(root,frame1,client_socket,message):
    for widget in frame1.winfo_children():
        widget.destroy()
    message = message + "... \nTry again..."
    Label(frame1, text=message, font=('Arial Rounded MT Bold', 16),bg=('#EE5C42')).grid(row = 1, column = 1)
    client_socket.close()
...
def voterLogin(root,frame1):
    ...
    Label(frame1, text="Voter Login", font=('Arial Rounded MT Bold', 25, 'bold')).grid(row = 0, column = 2, rowspan=1)
    Label(frame1, text="").grid(row = 1,column = 0)
    Label(frame1, text="Voter ID:      ", anchor="e", justify=LEFT,font=('Cambria', 12)).grid(row = 2,column = 0)
    ...
    sub = Button(frame1, text="LOGIN", width=10,font=('Oswald', 10, 'bold'),bg='#32CD32', command = lambda: log_server(root, frame1, client_socket, voter_ID.get(), password.get()))
